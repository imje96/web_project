        ...
        passwdck.addEventListener("input", function () {
            if (passwd.value === passwdck.value) {
                if (validatePassword(passwd.value)) {
                    message.textContent = "비밀번호가 일치하며 유효합니다.";
                    document.querySelector('input[type="submit"]').disabled = false; // 비밀번호 일치 시 회원가입 버튼 활성화
                } else {
                    message.textContent = "비밀번호는 영문과 숫자를 조합하여 최소 4글자 이상이어야 합니다.";
                    document.querySelector('input[type="submit"]').disabled = true; // 비밀번호 유효성 검사 실패 시 회원가입 버튼 비활성화
                }
            } else {
                message.textContent = "비밀번호가 일치하지 않습니다.";
                document.querySelector('input[type="submit"]').disabled = true; // 비밀번호 불일치 시 회원가입 버튼 비활성화
            }
        });
   ...
   /*  document.addEventListener("DOMContentLoaded", function () {
        ...
        passwdck.addEventListener("input", function () {
            ...
            // 비밀번호 형식 오류 추가
               ...
               message.text.content = "비밀번호 형식이 올바르지 않습니다.";
               document.querySelector('input[type="submit"]').disabled = false; // 비밀번호 형식이 맞지 않으면 회원가입 버튼 비활성화 
            }
        }  else {
                message.textContent = "비밀번호가 일치하지 않습니다.";
 ...
 */
        function isPasswordValid(password) {
          // 비밀번호는 조건은 영문 숫자로 구성하며 최소 4글자 이상 필요
          const regex = /^(?=.*[a-zA-Z])(?=.*\d).{4,}$/;
          return regex.test(password);
        }
      });
      ...
      if (message !== 'null' && message !== '') {
         ...
         <div class="form-container">
            <form action="join.member" method="post"
               onsubmit="return validateId() && validatePersonalId() && validateSimplePw() &&isPasswordValid() && validateAddress() && validateEmail()">
               <!--  <h2>회원가입</h2> -->
