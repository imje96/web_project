...
document.addEventListener("DOMContentLoaded", function () {
    ...
    passwd.addEventListener("input", function () {
    	if (!passwd.value.match(passwordValidation)) {
            ...
            submitBtn.disabled = false; // 비밀번호 형식이 맞으면 submit 버튼 활성화
        }
    });
...
/*     document.addEventListener("DOMContentLoaded", function () {
        ...
        let passwordValidation = /(?=.*\d)(?=.*[a-zA-Z])/; // 숫자와 알파벳이 모두 포함되어야 함 
        
        passwdck.addEventListener("input", function () {
        	  if (!passwd.value.match(passwordValidation)) {
                  message.textContent = "4자리 이상의 영문과 숫자로 입력해주세요."; // 형식이 올바르지 않을 경우 메시지
        ...
        document.querySelector('form').addEventListener("submit", function (event) {
            if (passwd.value !== passwdck.value || !passwd.value.match(passwordValidation)) {
                event.preventDefault(); // submit 동작 중지
                alert("비밀번호가 일치하지 않습니다."); // 비밀번호 불일치 또는 형식이 올바르지 않을 시 알림창 표시
            }
        });
    }); */
