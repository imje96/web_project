...
public class AccountInfoController extends HttpServlet {
	...
	private void actionDo(final HttpServletRequest request, final HttpServletResponse response)
		...
		/*
		 * String viewPage = null; String uri = request.getRequestURI();
		 * System.out.println("uri : " + uri);
		 * 
		 * String conPath = request.getContextPath(); System.out.println("conPath : " +
		 * conPath);
		 * 
		 ...
		 * 
		 * String memberId = request.getParameter("memberId"); String accountPassword =
		 * request.getParameter("accountPassword"); String nickname =
		 * request.getParameter("nickname"); int accountType =
		 * Integer.parseInt(request.getParameter("accountType"));
		 * AccountInfoRepository.createAccount(memberId, accountPassword, nickname,
		 * accountType); viewPage = "success.jsp"; } else {
		 * System.out.println("잘못된 command"); viewPage = "error.jsp"; // 또는 다른 적절한 에러
		 * 페이지로 초기화 }
		 * 
